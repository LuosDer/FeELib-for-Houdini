...
)
{
    ...
    //T defaultValue = static_cast<T>(defaults);
    ...
    else if (std::is_same<T, fpreal32>::value)
    {
        fpreal32 defaultValue;
        defaults.get(0, defaultValue);
    }
    else if (std::is_same<T, fpreal64>::value)
    {
        fpreal64 defaultValue;
        defaults.get(0, defaultValue);
    }
