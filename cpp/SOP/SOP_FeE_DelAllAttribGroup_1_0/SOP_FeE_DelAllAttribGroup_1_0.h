...
#ifndef __SOP_FeE_DelAllAttribGroup_1_0_h__
#define __SOP_FeE_DelAllAttribGroup_1_0_h__
...
class SOP_FeE_DelAllAttribGroup_1_0 : public SOP_Node
{
public:
    static PRM_Template *buildTemplates();
    static OP_Node *myConstructor(OP_Network *net, const char *name, OP_Operator *op)
    {
        OP_Node* newOp = new SOP_FeE_DelAllAttribGroup_1_0(net, name, op);
...
protected:
    SOP_FeE_DelAllAttribGroup_1_0(OP_Network *net, const char *name, OP_Operator *op)
        : SOP_Node(net, name, op)
    {
        mySopFlags.setManagesDataIDs(true);
    }
    
    ~SOP_FeE_DelAllAttribGroup_1_0() override {}
    ...
    const char* inputLabel(unsigned idx) const override
    {
        switch (idx)
        {
        case 0:     return "Geo with Attribute Group";
