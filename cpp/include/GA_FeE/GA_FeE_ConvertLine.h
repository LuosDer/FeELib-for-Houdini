...
)
{
    ...
    const GA_Storage finalStorage = storage == GA_STORE_INVALID ? GA_FeE_Type::getPreferredStorageI(geoPoint) : storage;
    //const GA_VertexGroup* creatingGroup = GA_FeE_VertexNextEquiv::addGroupVertexNextEquivNoLoop(geoFullTmp, "__topo_nextEquivValid", geoVtxGroup, subscribeRatio, minGrainSize);
    const GA_VertexGroup* const creatingGroup = GA_FeE_VertexNextEquiv::addGroupVertexNextEquivNoLoop(geoFullTmp, geoVtxGroup, finalStorage);
    //const GA_Attribute* dstptAttrib = geoPoint->findVertexAttribute("__topo_dstpt");
    const GA_RWHandleT<GA_Offset> dstptAttrib_h = geoFullTmp->findVertexAttribute(GA_FEE_TOPO_SCOPE, "__topo_dstpt");
