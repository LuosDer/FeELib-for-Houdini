...
#ifdef NDEBUG
using GFE_DEBUG_MODE = false;
#else
using GFE_DEBUG_MODE = true;
#endif
...
using GFE_MAX_LOOP_COUNT = std::numeric_limits<int32>::max();
using GFE_FIND_INVALID_INDEX = -1;




#if 0
    static constexpr int32 GFE_INVALID_OFFSET32 = std::numeric_limits<int32>::max(); // 2147483647
    static constexpr int64 GFE_INVALID_OFFSET64 = std::numeric_limits<int64>::max(); // 9223372036854775807
#else
    using GFE_INVALID_OFFSET32 = -1;
    using GFE_INVALID_OFFSET64 = -1;
#endif

using GFE_INVALID_OFFSET = GFE_INVALID_OFFSET32;



#if 0
    using GFE_TOPO_SCOPE = GFE_SCOPE_PRIVATE;
#else
    using GFE_TOPO_SCOPE = GA_AttributeScope::GA_SCOPE_PUBLIC;
#endif


template <typename T> static constexpr T           PI = 3.141592653589793238462643383279502884e+00 ;
template <>           static constexpr float32     PI = 3.141592653589793238462643383279502884e+00F; 
template <>           static constexpr float64     PI = 3.141592653589793238462643383279502884e+00 ;
template <>           static constexpr long double PI = 3.141592653589793238462643383279502884e+00L;
...
_INLINE_VAR constexpr bool isTuple = isIFVM<_Ty> ||
    ...
    namespace detail
    {
        ...
        {
            if constexpr(GFE_Core::isVector4<_Ty>)
                return _Ty(0, 0, 0, 0);
            else
                return _Ty(0.0);
        }
...
{
    if constexpr(GFE_Core::isVector<_Ty>)
...
template<typename _Ty>
using GFE_RWPageHandleT = GA_PageHandleT<_Ty, typename GFE_Core::get_value_type_t<_Ty>, true, true, GA_Attribute, GA_ATINumeric, GA_Detail>;

template<typename _Ty>
using GFE_ROPageHandleT = GA_PageHandleT<_Ty, typename GFE_Core::get_value_type_t<_Ty>, true, false, const GA_Attribute, const GA_ATINumeric, const GA_Detail>;
