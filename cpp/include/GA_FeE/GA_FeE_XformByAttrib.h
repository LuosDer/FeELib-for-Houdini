...
{
    UT_ASSERT_P(attribPtr);
    const GA_SplittableRange geoSplittableRange(GA_FeE_Range::getSplittableRangeByAnyGroup(geo, geoGroup, attribPtr->getOwner()));
...
{
    const GA_SplittableRange geoSplittableRange(GA_FeE_Range::getSplittableRangeByAnyGroup(geo, geoGroup, owner));
    ...
    const GA_Range geoRange = GA_FeE_Range::getRangeByAnyGroup(geo, geoGroup, owner);
...
{
    GOP_Manager gop;
    const GA_Group* const geoGroup = GA_FeE_Group::findOrParseGroupDetached(cookparms, geo, groupType, geoGroupName, gop);
    ...
    GA_FeE_Attribute::bumpDataId(geo, owner, attribToXformName);
}
