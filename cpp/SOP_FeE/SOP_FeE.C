...
// to provide SOP_FeEParms, an easy way to access parameter values from
// SOP_FeEVerb::cook with the correct type.
#include "SOP_FeE.proto.h"
...
using namespace SOP_FeE_Namespace;
...
const UT_StringHolder SOP_FeE::theSOPTypeName("FeE::attribScale::1.0"_sh);
...
newSopOperator(OP_OperatorTable *table)
{
    table->addOperator(new OP_Operator(
        SOP_FeE::theSOPTypeName,   // Internal name
        "FeE Attribute Scale",     // UI name
        SOP_FeE::myConstructor,    // How to build the SOP
        SOP_FeE::buildTemplates(), // My parameters
        1,                         // Min # of sources
        1,                         // Max # of sources
...
PRM_Template*
SOP_FeE::buildTemplates()
{
    static PRM_TemplateBuilder templ("SOP_FeE.C"_sh, theDsFile);
    return templ.templates();
}

class SOP_FeEVerb : public SOP_NodeVerb
{
public:
    SOP_FeEVerb() {}
    virtual ~SOP_FeEVerb() {}

    virtual SOP_NodeParms *allocParms() const { return new SOP_FeEParms(); }
    virtual UT_StringHolder name() const { return SOP_FeE::theSOPTypeName; }
    ...
    static const SOP_NodeVerb::Register<SOP_FeEVerb> theVerb;
};

// The static member variable definition has to be outside the class definition.
// The declaration is inside the class.
const SOP_NodeVerb::Register<SOP_FeEVerb> SOP_FeEVerb::theVerb;

const SOP_NodeVerb *
SOP_FeE::cookVerb() const 
{ 
    return SOP_FeEVerb::theVerb.get();
}
...
sopAttribOwner(SOP_FeEParms::AttribClass attribClass)
{
    using namespace SOP_FeEEnums;
    switch (attribClass)
    {
...
static GA_GroupType
sopGroupType(SOP_FeEParms::GroupType parmgrouptype)
{
    using namespace SOP_FeEEnums;
...
void
SOP_FeEVerb::cook(const SOP_NodeVerb::CookParms &cookparms) const
{
    auto &&sopparms = cookparms.parms<SOP_FeEParms>();
...
namespace SOP_FeE_Namespace {

} // End SOP_FeE_Namespace namespace
